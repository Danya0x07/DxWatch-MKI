...
#define NUM_PIXELS_Y    (DISPLAY_NUM_ROWS * 8)
...
static struct {
    ...
    enum GfxImageScale imageScale;
    bool inverse;
} brush = {GfxFontSize_7X5, GfxImageScale_X1, false};
...
{
    const uint8_t *bitmap = (const uint8_t *)&ascii_charset[(uint8_t)(c - 0x20)];
...
static void writeRam(uint8_t byte)
{
    if (brush.inverse)
        byte = ~byte;

    DISPLAY_WriteByte(byte);
    if (++cursor.x > DISPLAY_MAX_X) {
        cursor.x = 0;
        if (++cursor.p > DISPLAY_MAX_PG)
            cursor.p = 0;
        DISPLAY_SetCursor(cursor.x, cursor.p);
    }
}

static void DrawByte(uint8_t byte, uint8_t xScale, uint8_t yScale)
{
    uint8_t buffer[DISPLAY_NUM_ROWS] = {0};
    const uint_fast8_t startX = cursor.x;
    const uint_fast8_t startP = cursor.p;
    uint_fast8_t currX = startX;
    uint_fast8_t currP = startP;
    uint_fast8_t bitStatus;

    for (uint_fast8_t i = 0, bitNo = 0; i < 8 && bitNo < NUM_PIXELS_Y; i++) {
        bitStatus = byte & 1;
        for (uint_fast8_t j = 0; j < yScale; j++) {
            buffer[bitNo >> 3] |= bitStatus << (bitNo & 7);
            bitNo++;
        }
        byte >>= 1;
    }

    for (uint_fast8_t i = 0; i < yScale; i++) {
        currX = startX;
        SetRamAddr(currX, currP);
        for (uint_fast8_t j = 0; j < xScale; j++) {
            writeRam(buffer[i]);
            if (++currX > DISPLAY_MAX_X)
                break;
        }
        if (++currP > DISPLAY_MAX_PG)
            break;
    }
    SetRamAddr(currX, startP);
}
...
void GFX_PrintChar(char c)
{
    ...
    for (uint_fast8_t i = 0; i < 5; i++) {
        DrawByte(*bitmap++, brush.fontSize, brush.fontSize);
    }
    DrawByte(0, brush.fontSize, brush.fontSize);
}

void GFX_PrintString(const char *s)
{
    GFX_PrintStringInFrame(0, DISPLAY_NUM_COLS, DISPLAY_MAX_PG, s);
}

void GFX_PrintStringInFrame(uint8_t x0, uint8_t x1, uint8_t p1, const char *s)
{
    uint_fast8_t p = cursor.p;
    uint_fast8_t place = cursor.x / CHAR_WIDTH_PX / brush.fontSize;
    uint_fast8_t startPlace = x0 / CHAR_WIDTH_PX / brush.fontSize;
    uint_fast8_t endPlace = x1 / CHAR_WIDTH_PX / brush.fontSize;

    if (x1 > DISPLAY_NUM_COLS)
        x1 = DISPLAY_NUM_COLS;
    if (p1 > DISPLAY_MAX_PG)
        p1 = DISPLAY_MAX_PG;
    if (x0 > x1)
        return;
...
void GFX_DrawImage(uint8_t x0, uint8_t p0, const struct GfxImage *image)
{
    ...
    if (x0 > DISPLAY_MAX_X || p0 > DISPLAY_MAX_PG)
        return;

    for (uint_fast8_t i = 0, j = 0, p = p0; i < image->heightPg; i++) {
        x = x0;
        SetRamAddr(x, p);
        for (j = 0; j < image->widthPx; j++) {
            DrawByte(*bitmap++, brush.imageScale, brush.imageScale);
            x += brush.imageScale;
            if (x > DISPLAY_MAX_X)
                break;
        }
        bitmap += image->widthPx - j - (x >= DISPLAY_NUM_COLS);
        p += brush.imageScale;
        if (p > DISPLAY_MAX_PG)
            break;
    }
...
void GFX_Clear(void)
{
    GFX_ClearRect(0, 0, DISPLAY_MAX_X, DISPLAY_MAX_PG);
}
...
void GFX_ClearRect(uint8_t x0, uint8_t p0, uint8_t x1, uint8_t p1)
{
    ...
    for (uint_fast8_t i = p0; i <= p1; i++) {
        DISPLAY_SetCursor(x0, i);
        for (uint_fast8_t j = x0; j <= x1; j++) {
            DISPLAY_WriteByte(0);
        }
    }
    GFX_SetCursor(x0, p0);
